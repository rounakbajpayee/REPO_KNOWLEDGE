...
def mock_connect():
    ...
    with patch("repo_knowledge.postgres_store.pgpool.ThreadedConnectionPool",
...
def test_upsert_chunks(mock_connect):
    ...
    assert exec_args[1] == (
        "uuid-xyz", 101, "my_proj", "src/main.py", "python",
        "class", "MyClass", "class MyClass: pass", 1, 10
    )
...
def test_health_check_returns_false_on_failure():
    ...
    with patch("repo_knowledge.postgres_store.psycopg2.connect", side_effect=Exception("Connection failed")):
        store = PostgresStore()
