...
def query_llm(model: str, system_prompt: str, user_prompt: str) -> dict:
    ...
    if start == -1 or end == -1 or end < start:
        raise RuntimeError(
            f"LLM did not output a valid JSON block. Output was:\n{response_text}"
        )
...
@click.command()
@click.option(
    "--diff", is_flag=True, help="Analyze unstaged and staged workspace changes."
)
@click.option("--commits", default=0, help="Analyze the last N commits.")
@click.option(
    "--transcript",
    is_flag=True,
    help="Find and parse the latest Antigravity session transcript.",
)
@click.option(
    "--topic",
    default=None,
    help="Override topic name (forces all entries into this topic).",
)
def main(diff: bool, commits: int, transcript: bool, topic: str | None) -> None:
    ...
    if len(source_text) > 8000:
        click.secho(
            "  [WARN] Source text truncated to 8000 chars for LLM analysis",
            fg="yellow",
            err=True,
        )
        source_text = (
    ...
    if transcript:
        ...
        if not appdata:
            click.secho(
                "  [WARNING] APPDATA environment variable not found.", fg="yellow"
            )
        else:
            ...
            if not brain_dir.exists():
                ...
                if not transcripts:
                    click.secho(
                        "  [WARNING] No transcript.jsonl logs found under brain/.",
                        fg="yellow",
                    )
                else:
                    ...
                    try:
                        ...
                        click.secho(
                            f"  [ERROR] Could not read transcript: {e}", fg="red"
                        )

    if not source_text.strip() or len(source_text.strip()) < 20:
        click.secho(
            "No significant source data gathered to extract decisions from. Exiting.",
            fg="yellow",
        )
        sys.exit(0)
    ...
    if not target_topic:
        click.secho(
            "Error: No topic generated by LLM and no --topic override provided.",
            fg="red",
        )
        sys.exit(1)
    ...
    click.echo(
        f"Logging decision '{decision_data.get('name')}' under topic '{target_topic}'..."
    )
